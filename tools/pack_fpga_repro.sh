...
DEST1_ABS_PATH="$(cd "$DEST1_DIR" && pwd -P)"
DEST2_ABS_PATH="NOT_PROVIDED"
if [[ -n "$DEST2_DIR" && "$DEST2_ENABLED" == true ]]; then
  DEST2_ABS_PATH="$(cd "$DEST2_DIR" && pwd -P)"
fi
...
if [[ "$DRY_RUN" == false ]]; then
  {
    ...
    echo "DEST1_ABS_PATH=$DEST1_ABS_PATH"
    echo "DEST2_ABS_PATH=$DEST2_ABS_PATH"
...
if [[ -n "$DEST2_DIR" && "$DEST2_ENABLED" == true ]]; then
  ...
  dest2_delivered=()
  if [[ "$DRY_RUN" == true ]]; then
    log "DEST2: chmod -R 777 \"$DEST2_DIR\""
  else
    log "DEST2: chmod -R 777 \"$DEST2_DIR\""
    chmod -R 777 "$DEST2_DIR" || log "WARN: chmod -R 777 failed for DEST2_DIR"
  fi
  shopt -s nullglob
  ...
  if [[ ${#redcomp_tarballs[@]} -eq 0 ]]; then
    ...
    for tarball in "${redcomp_tarballs[@]}"; do
      ...
      if [[ "$DRY_RUN" == true ]]; then
        log "DEST2: copy $tar_base -> dest2, extract into DEST2 root, remove tar.gz, chmod -R 777"
        dest2_delivered+=("$tar_base")
        continue
      fi

      cp -f "$tarball" "$dest2_tar"
      dest2_delivered+=("$tar_base")
      if ! tar -xzf "$dest2_tar" -C "$DEST2_DIR"; then
      ...
      dest2_results+=("$tar_base: extracted_to_dest2_root")
    done
  fi
  ...
  if [[ "$DRY_RUN" == false ]]; then
    ...
    {
      echo "This directory is the FPGA server delivery area and corresponds to the DEST1 archive."
      echo "BUILDID: $BUILD_ID"
      echo "TIME: $TIME_NOW"
      echo "HOST: $HOSTNAME"
      echo "USER: $USER_NAME"
      ...
      if [[ ${#dest2_results[@]} -gt 0 ]]; then
        printf '%s\n' "${dest2_results[@]}" | sed 's/:.*$//; s/^/- /'
      else
        echo "- (none)"
      fi
      echo "Extracted: extracted into DEST2 root (chmod 777)"
...
if [[ "$DRY_RUN" == false ]]; then
  dest2_delivered_list="NONE"
  if [[ -n "${dest2_delivered[*]:-}" ]]; then
    dest2_delivered_list="$(IFS=,; echo "${dest2_delivered[*]}")"
  fi

  if [[ -n "$DEST2_DIR" && "$DEST2_ENABLED" == true ]]; then
    dest1_link_file="$DEST1_DIR/LINK_DEST2.txt"
    dest2_link_file="$DEST2_DIR/LINK_DEST1.txt"
    extract_mode="dest2_root"

    {
      ...
      echo "DEST1_ABS_PATH=$DEST1_ABS_PATH"
      echo "DEST2_ABS_PATH=$DEST2_ABS_PATH"
      echo "FULL_REPRO_TARBALL=$TAR_NAME"
      echo "FULL_REPRO_TARBALL_SHA256=$TAR_SHA256"
      echo "DEST2_DELIVERED_TARBALLS=$dest2_delivered_list"
      echo "DEST2_EXTRACT_MODE=$extract_mode"
      echo "NOTE=dest1/dest2 are a paired bundle; use paths above for lookup"
    } > "$dest1_link_file"
    ...
    chmod 644 "$dest1_link_file" "$dest2_link_file" || log "WARN: chmod 644 failed for link files"

    dest1_readme="$DEST1_DIR/README_DEST1_LINK.txt"
    {
      echo "DEST1_ABS_PATH=$DEST1_ABS_PATH"
      echo "DEST2_ABS_PATH=$DEST2_ABS_PATH"
    } > "$dest1_readme"
